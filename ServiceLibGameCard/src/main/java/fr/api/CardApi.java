...
public class CardApi {
    private static final String BASE_URL = "http://service-card";
...
/*
    ...
    public ResponseMessage<CardOut> buyCardByUser(@PathVariable("cardId") int cardId, Integer userId) {

        RestTemplate restTemplate = new RestTemplate();
        String url = BASE_URL + "/buy-card/" + cardId;
        ...
        if (response.getStatusCode().is2xxSuccessful()) {
            ...
            // Handle error case
            throw new RuntimeException("Failed to buy card with ID: " + cardId);
        }
    }
